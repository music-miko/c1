...
func main() {
	...
	handlers.LoadCloneModules(client)

	clone.Init(manager, config.ApiId, config.ApiHash, tdDir, func(cl *gotdbot.Client) {
		handlers.LoadModules(cl)
		handlers.LoadCloneOwnerModules(cl)
	})
	clone.Instance.RestoreAll()
