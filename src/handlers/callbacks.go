...
func playCallbackHandler(c *td.Client, cb *td.UpdateNewCallbackQuery) error {
	...
	case strings.Contains(data, "play_skip"):
		if err := vc.Calls.PlayNextFor(c.Me.Id, c, chatID); err != nil {
	...
	case strings.Contains(data, "play_stop"):
		if err := vc.Calls.StopFor(c.Me.Id, chatID, false); err != nil {
	...
	case strings.Contains(data, "play_pause"):
		if _, err = vc.Calls.PauseFor(c.Me.Id, chatID); err != nil {
	...
	case strings.Contains(data, "play_resume"):
		if _, err := vc.Calls.ResumeFor(c.Me.Id, chatID); err != nil {
	...
	case strings.Contains(data, "play_mute"):
		if _, err := vc.Calls.MuteFor(c.Me.Id, chatID); err != nil {
	...
	case strings.Contains(data, "play_unmute"):
		if _, err := vc.Calls.UnmuteFor(c.Me.Id, chatID); err != nil {
