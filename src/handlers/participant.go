...
func handleParticipant(client *gotdbot.Client, update *gotdbot.UpdateChatMember) error {
	...
	assistant, _, err := vc.Calls.GetGroupAssistantFor(client.Me.Id, chatID)
	if err != nil {
...
func onJoin(
	...
	if userID == client.Me.Id {
		...
		if reg, _ := db.Instance.GetClone(client.Me.Id); reg != nil {
			_ = db.Instance.AddCloneChat(client.Me.Id, chatID)
		}
	}
...
func onLeave(client *gotdbot.Client, chatID, userID, assistantID int64) error {
	...
	if userID == client.Me.Id {
		if err := vc.Calls.StopFor(client.Me.Id, chatID, true); err != nil {
			client.Logger.Error("Failed to stop VC after leave", "error", err)
		}
		_ = db.Instance.RemoveCloneChat(client.Me.Id, chatID)
	}
