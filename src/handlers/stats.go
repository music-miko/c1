...
func statsHandler(c *td.Client, m *td.Message) error {
	if !isDev(c, m) {
		if canEdit(c, m) {
			return cloneOwnerStatsHandler(c, m)
		}
		return td.EndGroups
	}
...
// cloneOwnerStatsHandler is the scoped /stats view for a clone owner (or
// clone sudo user): only that clone's own status, no global DB/system
// internals, mirroring anony/clone/plugins/stats.py.
func cloneOwnerStatsHandler(c *td.Client, m *td.Message) error {
	activeVC := len(vc.Calls.ActiveChatsFor(c.Me.Id))
	uptime := time.Since(startTime).Round(time.Second).String()
	...
	text := fmt.Sprintf(
		...
		c.Me.Usernames.EditableUsername, uptime, activeVC,
	)
